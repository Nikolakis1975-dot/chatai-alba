...
// =========================================== ⚡ ULTIMATE KNOWLEDGE FIX v2 ================================================

console.log('⚡ ULTIMATE KNOWLEDGE FIX v2 - Loading...');

// ✅ 1. KRIJO NJË SISTEM TË PLOTË TË RI
(function() {
    console.log('🔧 Creating ultimate system v2...');
    
    // ✅ 2. MBIVENDOS ABSOLUTISHT TË GJITHA
    function overrideEverything() {
        console.log('🎯 Overriding everything...');
        
        // ✅ A. MBIVENDOS SENDMESSAGE
        ...
        if (typeof processCommand === 'function') {
            ...
            console.log('✅ processCommand overridden');
        }
    ...
    // ✅ 3. KONFIGURO INPUT
    function setupInputUltimate() {
        const input = document.getElementById('user-input');
        const button = document.getElementById('send-btn');
        
        if (!input || !button) {
            setTimeout(setupInputUltimate, 500);
            return;
        }
        ...
        newButton.addEventListener('click', handleSendUltimate);
        
        console.log('✅ Input configured');
    }
    
    // ✅ 4. HANDLE SEND
    async function handleSendUltimate() {
        const input = document.getElementById('user-input');
        const message = input ? input.value.trim() : '';
        
        if (!message) return;
        
        console.log('💬 [HANDLE] Sending:', message.substring(0, 20));
    ...
    // ✅ 5. PROCESO MESAZH
    async function processMessageUltimate(message) {
        console.log('🔧 [PROCESS] Message:', message.substring(0, 20));
        
        // ✅ /meso KOMANDA
        if (message.startsWith('/meso')) {
            await handleMesoUltimate(message);
            return;
        }
        
        // ✅ KËRKO NJOHURI
        const hasKnowledge = await searchKnowledgeUltimate(message);
        if (hasKnowledge) return;
        
        // ✅ NËSE NUK KA NJOHURI, DËRGO TE AI
        await sendToAIUltimate(message);
    }
    
    // ✅ 6. HANDLE /meso
    async function handleMesoUltimate(message) {
        const parts = message.substring(6).split('|');
        
        if (parts.length !== 2) {
            showMessageUltimate('❌ Format: /meso pyetja|përgjigja', 'bot');
            return;
        }
        ...
        if (!question || !answer) {
            showMessageUltimate('❌ Plotëso pyetjen dhe përgjigjen', 'bot');
            return;
        }
        
        // ✅ SHFAQ
        showMessageUltimate(`💾 "${question}" → "${answer}"`, 'bot');
    ...
    // ✅ 7. KËRKO NJOHURI
    async function searchKnowledgeUltimate(message) {
        // ✅ PASTOR
        ...
        try {
            ...
            if (data.success && data.found && data.answer) {
                console.log('✅✅✅ FOUND:', data.answer);
                // ✅ SHFAQ VETËM PËRGJIGJEN
                showMessageUltimate(data.answer, 'bot');
                return true;
            }
        } catch (error) {
            console.log('ℹ️ No knowledge');
        }
    ...
    // ✅ 8. DËRGO TE AI
    async function sendToAIUltimate(message) {
        console.log('🤖 [AI] Sending:', message.substring(0, 20));
        ...
        try {
            ...
            if (data.success && data.response) {
                showMessageUltimate(data.response, 'bot');
            } else {
                showMessageUltimate('❌ ' + (data.error || 'Gabim'), 'bot');
            }
        } catch (error) {
            console.error('❌ AI error:', error);
            showMessageUltimate('❌ Gabim në lidhje', 'bot');
        }
    }
    
    // ✅ 9. SHFAQ MESAZH
    function showMessageUltimate(text, sender) {
        if (typeof window.addMessage === 'function') {
            window.addMessage(text, sender);
        }
    }
    
    // ✅ 10. DISABLE SISTEMIN E VJETËR PLOTËSISHT
    function disableOldSystem() {
        console.log('🛑 Disabling old system...');
        
        const blocker = document.createElement('script');
        blocker.textContent = `
            console.log('⚡ ULTIMATE BLOCKER v2 activated');
            
            // 🛑 ÇAKTIVIZO TË GJITHA FUNKSIONET E VJETRA
            window.oldSystemDisabled = true;
            
            console.log('✅ Old system completely disabled');
        `;
    ...
    // ✅ 11. INICIALIZO
    setTimeout(() => {
        console.log('🚀 Starting ultimate system v2...');
        ...
        setTimeout(() => {
            ...
            console.log('✅✅✅ ULTIMATE SYSTEM v2 READY!');
            
            // ✅ SHFAQ NJOFTIM
            showMessageUltimate('⚡ **Ultimate System v2** aktiv! Tani /meso dhe kërkimi punojnë 100%.', 'system');
