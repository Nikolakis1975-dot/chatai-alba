...
// ============================= ✅ FIX PËR NJOHURITË E RUAJTURA & LLOGARITJE MATEMATIKE ================================

console.log('🔧 Duke aktivizuar sistemin e njohurive dhe llogaritjeve...');

// ✅ MBIVENDOS FUNKSIONIN sendMessage PËR TË KAPUR NJOHURITË & LLOGARITJE
const originalSendMessage = window.sendMessage;
...
window.sendMessage = async function() {
    ...
    console.log('🚀 [SEND-MESSAGE-ENHANCED] Mesazh:', message);

    // ✅ KONTROLLO NËSE ËSHTË KOMANDË
    if (message.startsWith('/')) {
        console.log('🎯 [SEND-MESSAGE-ENHANCED] Komandë, duke përdorur procesin normal...');
        await originalSendMessage.call(this);
        return;
    }

    // ✅ 1. KONTROLLO NËSE KA NJOHURI TË RUAJTURA PËR KËTË PYETJE
    try {
        console.log('🔍 [SEND-MESSAGE-ENHANCED] Duke kërkuar njohuri të ruajtura...');
        ...
        if (window.currentUser && window.currentUser.id) {
            ...
            if (response.ok) {
                const data = await response.json();
                console.log('📊 [SEND-MESSAGE-ENHANCED] Përgjigja e serverit:', data);
                
                if (data.answer && data.answer !== null) {
                    console.log('✅ [SEND-MESSAGE-ENHANCED] Gjetëm përgjigje të ruajtur:', data.answer);
        ...
        console.log('ℹ️ [SEND-MESSAGE-ENHANCED] Nuk ka përgjigje të ruajtur ose gabim:', error.message);
    }

    // ✅ 2. KONTROLLO PËR LLOGARITJE MATEMATIKE
    try {
        console.log('🧮 [SEND-MESSAGE-ENHANCED] Duke kontrolluar për llogaritje...');
        
        // Përdor funksionin tryCalculate nga script.js nëse ekziston
        if (typeof tryCalculate !== 'undefined') {
            const mathResult = tryCalculate(message);
            if (mathResult !== null) {
                console.log('✅ [SEND-MESSAGE-ENHANCED] Llogaritje e gjetur:', mathResult);
        ...
        for (const pattern of mathPatterns) {
            ...
            if (match && match[1]) {
                ...
                if (expression.length > 2) {
                    console.log('🔢 [SEND-MESSAGE-ENHANCED] Shprehje matematikore:', expression);
                    ...
                    try {
                        ...
                        console.log('✅ [SEND-MESSAGE-ENHANCED] Llogaritja u krye:', formattedResult);
                        addMessage(`🧮 **Rezultati**: ${formattedResult}\n\n📝 *Shprehja: ${expression}*`, 'bot');
                        userInput.value = '';
                        return;
                        
                    } catch (evalError) {
                        console.log('❌ [SEND-MESSAGE-ENHANCED] Llogaritja dështoi:', evalError.message);
                    }
                    break;
                }
            }
        ...
        console.log('❌ [SEND-MESSAGE-ENHANCED] Gabim në kontrollin e llogaritjeve:', error);
    }

    // ✅ 3. NËSE NUK GJETËM NJOHURI OSE LLOGARITJE, DËRGO TE SERVERI
    console.log('🔄 [SEND-MESSAGE-ENHANCED] Duke dërguar mesazh normal te serveri...');
    await originalSendMessage.call(this);
};

// ✅ FIX PËR MOTORIN NË /ndihmo
function getActiveEngine() {
    if (window.aiEngineStatus?.openai) return 'openai';
    if (window.aiEngineStatus?.gemini) return 'gemini';
...
setTimeout(() => {
    ...
    console.log('- Motor aktiv:', getActiveEngine());
}, 2000);
