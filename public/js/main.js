...
console.log('🔧 Duke inicializuar sistemin e komandave...');
...
window.sendMessage = async function() {
    ...
    // ✅ KONTROLLO NËSE ËSHTË KOMANDË - DHE THIRR PROCESSCOMMAND
    if (message.startsWith('/')) {
        console.log('🎯 [SEND-MESSAGE-FIX] Komandë e zbuluar:', message);
        ...
        try {
            ...
            loadingDiv.className = 'message bot';
            loadingDiv.innerHTML = '<div class="message-text">⏳ Po procesoj komandën...</div>';
            chat.appendChild(loadingDiv);
            chat.scrollTop = chat.scrollHeight;

            // ✅ THIRR FUNKSIONIN PROCESSCOMMAND QË E KE NË MAIN.JS
            if (typeof processCommand !== 'undefined') {
                console.log('✅ [SEND-MESSAGE-FIX] Duke thirrur processCommand...');
                await processCommand(message);
            } else {
                console.log('❌ [SEND-MESSAGE-FIX] processCommand nuk ekziston');
                ...
                if (data.success) {
                    ...
                    // FALLBACK MANUAL
                    const fallbackResponse = getManualCommandResponse(message);
            ...
            console.error('❌ [SEND-MESSAGE-FIX] Gabim në komandë:', error);
            document.getElementById('command-loading')?.remove();
    ...
    console.log('🔄 [SEND-MESSAGE-FIX] Duke përdorur funksionin origjinal...');
    await originalSendMessage.call(this);
};

// ✅ FUNKSIONI PËR PJEGJIGJE MANUALE TË KOMANDAVE (FALLBACK)
function getManualCommandResponse(command) {
    const commands = {
        '/ndihmo': `👑 **SISTEMI I KOMANDAVE - RRUFE-TESLA** 👑
...
🔧 **Të gjitha komandat janë operative!**`,

        '/wiki': `🌐 **KOMANDA /wiki**

💡 Kjo komandë kërkon në Wikipedia për informacione.
...
💡 Kjo komandë jep informacion moti për një qytet.
...
*Duke marrë informacionin...*`
    };

    return commands[command] || `❌ **Komandë e panjohur:** ${command}\n\n💡 Shkruaj /ndihmo për të parë të gjitha komandat.`;
}

console.log('✅ Sistemi i komandave u inicializua në main.js!');
