...
// ======================================== ✅ KNOWLEDGE CHECK - MINIMAL & SAFE ======================================

console.log('🧠 [KNOWLEDGE-MINIMAL] Duke ngarkuar...');

// 1. Funksioni për të kontrolluar njohuritë
async function checkKnowledgeSimple(message) {
    try {
        console.log('🔍 [CHECK] Duke kontrolluar për:', message);
        
        // Kontrollo nëse ka user
        if (!window.currentUser || !window.currentUser.id) {
            console.log('⚠️ [CHECK] Nuk ka currentUser');
            return false;
        }
        ...
        console.log('🌐 [CHECK] Duke thirrur API për user', userId, ':', question);
        
        // Kërko në database
        const response = await fetch(`/api/chat/knowledge/${userId}/${encodeURIComponent(question)}`);
        const data = await response.json();
        
        console.log('📊 [CHECK] Përgjigja:', data);
        
        if (data.answer && data.answer !== 'null') {
            console.log('✅✅✅ [CHECK] GJETËM PËRGJIGJE:', data.answer);
        ...
        console.log('❌ [CHECK] Nuk u gjet përgjigje');
        return false;
        
    } catch (error) {
        console.log('🔥 [CHECK] Gabim:', error.message);
        return false;
    }
}

// 2. INTEGRIMI I THJESHTË - NUK PRISH ASGJË
function setupSimpleKnowledgeCheck() {
    console.log('⚙️ [SETUP] Duke setup...');
    
    const sendBtn = document.getElementById('send-btn');
    const userInput = document.getElementById('user-input');
    
    if (!sendBtn || !userInput) {
        console.log('⚠️ [SETUP] Elementet nuk u gjetën');
        setTimeout(setupSimpleKnowledgeCheck, 1000);
        return;
    }
    
    console.log('✅ [SETUP] Elementet u gjetën');
    
    // Shto një event listener TË RI pa prishur ato ekzistuese
    ...
    sendBtn.addEventListener('click', async function(e) {
        ...
        if (!message || message.startsWith('/')) {
            // Për komanda, lëre të vazhdojë normalisht
            if (originalClick) originalClick.call(this, e);
            return;
        }
        
        console.log('💬 [CLICK] Përdoruesi shkroi:', message);
        
        // Kontrollo nëse ka njohuri
        const hasKnowledge = await checkKnowledgeSimple(message);
        
        if (hasKnowledge) {
            console.log('✅ [CLICK] Përdorëm njohuri, nuk dërgojmë te AI');
            userInput.value = ''; // Pastro input
            return;
        }
        
        console.log('🤖 [CLICK] Nuk gjetëm njohuri, dërgo normalisht');
        if (originalClick) originalClick.call(this, e);
    });
    
    // Gjithashtu për Enter key
    userInput.addEventListener('keypress', async function(e) {
        if (e.key === 'Enter') {
            ...
            e.preventDefault(); // Parandalo refresh
            
            console.log('↵ [ENTER] Përdoruesi shkroi:', message);
            
            // Kontrollo njohuritë
            const hasKnowledge = await checkKnowledgeSimple(message);
            
            if (!hasKnowledge) {
                console.log('🤖 [ENTER] Nuk gjetëm njohuri, dërgo normalisht');
                // Duhet të gjesh mënyrën se si dërgohet normalisht me Enter
                // Mund të jetë: if (typeof sendMessage === 'function') sendMessage();
            }
        }
    });
    
    console.log('🎉 [SETUP] Sistemi minimal u setup!');
}

// 3. TEST BUTON - PËR DEBUG
function addTestButton() {
    ...
    testBtn.innerHTML = '🧪 Test Knowledge';
    ...
    testBtn.onclick = async () => {
        ...
        const testQuestion = 'si shkoni si kaloni miku im?';
        console.log('Duke testuar për:', testQuestion);
        await checkKnowledgeSimple(testQuestion);
    };
    
    header.appendChild(testBtn);
    console.log('🔧 [TEST] Butoni test u shtua');
}

// 4. INICIALIZO
setTimeout(() => {
    console.log('🚀 [INIT] Duke inicializuar...');
    setupSimpleKnowledgeCheck();
    addTestButton();
    
    // Testo sistemin
    console.log('📊 [STATUS] Sistemi i ngarkuar:');
    console.log('- checkKnowledgeSimple:', typeof checkKnowledgeSimple);
    console.log('- currentUser.id:', window.currentUser?.id);
    
}, 2000);

console.log('🎯 [KNOWLEDGE] Sistemi minimal i ngarkuar');
