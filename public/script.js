...
// ======================================================
// 💾 SISTEMI I RI PËR RUAJTJE MESAZHESH NË DATABASE
// ======================================================

// ✅ FUNKSION I RI PËR RUAJTJE MESAZHESH NË DATABASE
async function saveMessageToDatabase(content, sender, userId = 'user-1') {
    try {
        console.log(`💾 Duke ruajtur mesazh në database: ${sender} - ${content.substring(0, 50)}...`);
        ...
        if (response.ok) {
            console.log('✅ Mesazh u ruajt në database:', result.id);
            return true;
        } else {
            console.error('❌ Gabim në ruajtjen e mesazhit:', result.error);
            return false;
        }
    } catch (error) {
...
// ✅ FUNKSION I RI PËR MBISHTRESË TË addMessage - Ruan në database
const originalAddMessage = window.addMessage;

window.addMessage = async function(content, sender, saveToDatabase = true) {
    // ✅ THIRRE FUNKSIONIN ORIGJINAL
    if (originalAddMessage) {
        originalAddMessage(content, sender);
    } else {
        // Fallback nëse nuk ekziston funksioni origjinal
        const chat = document.getElementById('chat');
        if (!chat) return;
    ...
    // ✅ RUAJ NË DATABASE NËSE ËSHTË MESAZH I RI
    if (saveToDatabase && sender !== 'system') {
        const userId = window.currentUser?.id || 'user-1';
        await saveMessageToDatabase(content, sender, userId);
    }
}

// ✅ FUNKSION I RI PËR MBISHTRESË TË sendMessage - Ruan në database
...
window.sendMessage = async function() {
    ...
    // ✅ SHTO MESAZHIN E PËRDORUESIT NË UI DHE DATABASE
    await window.addMessage(message, 'user', true);
    ...
    try {
        ...
        if (data.success) {
            // ✅ SHTO PËRGJIGJEN E BOTIT NË UI DHE DATABASE
            await window.addMessage(data.response, 'bot', true);
        } else {
            await window.addMessage('❌ ' + (data.response || 'Gabim në server'), 'bot', true);
        }
    } catch (error) {
        console.error('❌ Gabim:', error);
        await window.addMessage('❌ Gabim në lidhje me serverin', 'bot', true);
    }
}

// ✅ FUNKSION NDIHMËS PËR FORMATIM MESAZHESH
...
console.log('✅ Sistemi i ri për ruajtje mesazhesh në database u ngarkua!');
